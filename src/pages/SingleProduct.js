...
function SingleProduct() {
	...
	const getProduct = async () => {
		axios.get(`${siteURL}/wp-json/wc/v2/products/13`, { headers: { "Authorization": `Bearer ${ProductData.token}` } })
			.then(
				response => {
				...
				product ?
					<div>
						<div className="main-div">
							<div className="product-img-row">
								<div className="product-main-img">
									<img onClick={popImg} id="productimage1" className="product-img1" src={product.images[0].src} alt={product.images[0].name} />
								</div>
								<div className="product-sub-img">
									<img onClick={prodImg} id="productimage2" className="product-img2" src={product.images[1].src} alt={product.images[1].name} />
									<img onClick={prodImg} id="productimage3" className="product-img3" src={product.images[1].src} alt={product.images[1].name} />
									<img onClick={prodImg} id="productimage4" className="product-img4" src={product.images[1].src} alt={product.images[1].name} />
								</div>
								...
								<div className="product-details-sub-row">
									...
									<div className="product-details-div">
										<div className="plank-size-div">
											...
											<div>
												<p id="plankSize">{product.attributes[0].options[0]}</p>
											</div>
										</div>
										<div className="box-contains-div">
											...
											<div>
												<p id="boxContains">{product.attributes[1].options[0]}</p>
											</div>
										</div>
										<div className="price-sqft-div">
											...
											<div>
												<p id="priceSqft">{product.attributes[2].options[0]}</p>
											</div>
										</div>
										<div className="price-box-div">
											...
											<div>
												<p id="priceBox">{product.attributes[3].options[0]}</p>
											</div>
										</div>
										<div className="features-div">
											...
											<div>
												<p id="features">{product.attributes[4].options[0]}<br/>{product.attributes[0].options[1]}</p>
											</div>
										</div>
										...
										<div className="total-price-div">
											...
											<div className="price">
												<div>
													<p>Rs.</p><p id="productPrice">{product.price}</p><p>/-</p>
												</div>
										...
										<div className="cart-div">
											<div>
												<button key={product.id} data-index={product.id} onClick={addToCart} type="button">Add to cart</button>
											</div>
											<div>
												<button onClick={GoToCart} type="button">Go to cart</button>
											</div>
										</div>
						...
						<div id="imagePopup" className="img-popup">
							...
							<img className="popped-up-img" id="poppedupImg" alt="close" />
						</div>
					</div>
				:
					<p>Loading...</p>
